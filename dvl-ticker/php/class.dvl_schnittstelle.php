...
class dvl_schnittstelle{
	...
	private $dvlStatitikURL =  array(
										'bl1m' => 'http://live.volleyball-bundesliga.de/2011/Men/&',	
										'bl1f' => 'http://live.volleyball-bundesliga.de/2011/Women/&',
										'bl2nm'=> 'http://live.volleyball-bundesliga.de/2011/escore/',
										'bl2sm'=> 'http://live.volleyball-bundesliga.de/2011/escore/',
										'bl2sf'=> 'http://live.volleyball-bundesliga.de/2011/escore/',
										'bl2nf'=> 'http://live.volleyball-bundesliga.de/2011/escore/'
	
	);
	private $tmpdir  = "/tmp/";
	private $liveTickerSekBevorSpielBeginn = 12000;
		...
		for ($j=0;$j<$tage;$j++) 
		{
			...
			if (!empty($DVL)) 
			{
				...
				{                
					...
					$html 		='"'.$spielBeginnTimeStamp.'-'.$result->spielnr.'": {  "html":"'. $template.'","spielnummer":'.$result->spielnr.', "datenquelle":"DVL-Schhnittstelle-results",  "last_update": '.$date->getTimestamp().'   }';
					//$html 		='"'.$result->spielnr.'": {  "html":"'. $template.'",    "last_update": '.$date->getTimestamp().'   }';
					//echo $spielBeginnTimeStamp;
					...
					if ($result->ergebnis != '0:0')   // Spiel schon beendet
					{
						...
						If ( $this->dvlStatitikURL[$liga]) {
							$html = str_replace ('###STATISTIKBUTTON###', '<a href=\'' . $this->dvlStatitikURL[$liga] . $result->spielnr .'.pdf' . '\' class=\'statistik_aktiv\'>&nbsp;</a>', $html );
						} else  {
					...
					elseif ($result->ergebnis == '0:0')   // Spiel noch nicht beendet
					{
						...
						if (  ( $spielBeginnTimeStamp - $this->liveTickerSekBevorSpielBeginn ) < time() ) {  // Spiel wird gerade gespeilt
							   ...
							   $DVlSpiel = simplexml_load_file('http://interface.dvl-datenbank.de/givemedata.php?kennung=9O73V49Ys6&job=spielplan&spielnr='.$result->spielnr);
							   
							    $daten_aus_soap = erstelleStatistik( $DVlSpiel->einzelspiel->spielid, $result->heimid, $result->gastid,$liga,$this->teamKurz[intval($result->heimid)], $this->teamKurz[intval($result->gastid)],str_replace( '.20' ,'.' ,$result->datum) . ' ' . $result->spielbeginn . 'h');
								$html 		='"'.$spielBeginnTimeStamp.'-'.$result->spielnr.'": {  "html":"'.$daten_aus_soap.'","spielnummer":'.$result->spielnr.',"datenquelle":"soap"  ,  "last_update": '.$date->getTimestamp().'   }';
